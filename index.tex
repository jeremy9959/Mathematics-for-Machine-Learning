...
\begin{equation}\phantomsection\label{eq-LSAnswer}{ \begin{aligned} m &=
...
\begin{equation}\phantomsection\label{eq-LSAnswer2}{
...
in \textbf{?@eq-multivariate}, and \(b\) is the ``intercept'', then the
...
E = \sum\_\{j=1\}\^{}\{N\} (Y\_j-\sum{s=1}\^{}\{k+1\}
X\{js\}M\{s\})\^{}2
...
\df{M_t}E = -2\sum\emph{\{j=1\}\^{}\{N\}
X}\{jt\}(Y*\{j\}-\sum\emph{\{s=1\}\^{}\{k+1\} X}\{js\}M\_\{s\})
...
\begin{equation}\phantomsection\label{eq-Msolution}{ M =
D^{-1}X^{\intercal}Y }\end{equation} and the ``predicted value''
\(\hat{Y}\) for \(Y\) is \$\$

\hat{Y} = XD\textsuperscript{\{-1\}X}\{\intercal\}Y.
...
(see \textbf{?@eq-projection}): \[ \hat{Y} = XD^{-1}X^{\intercal}Y.  \]
...
to see that \[ M*{0} = D*{0}^{-1}X*{0}^{\intercal}Y. \] The \(b\) value
we are interested in is the last entry \(m*{k+1}\) in \(M_{0}\). From
...
\(D_{0}^{-1}X_{0}^{\intercal}Y\) is \[ \mu*{Y} =
...
\begin{equation}\phantomsection\label{eq-variancedot}{
...
\begin{equation}\phantomsection\label{eq-covariancedot}{
...
\begin{equation}\phantomsection\label{eq-rxy}{
...
\subsection{Visualizing the covariance matrix}\label{sec-visualizecovar}
...
\begin{equation}\phantomsection\label{eq-linearscore}{
...
\(a_1,\ldots a_k\) is \begin{equation}\phantomsection\label{eq-ada}{
...
}\end{equation} More generally, if \(S_{1}\) and \(S_{2}\) are scores
with weights \(a_1,\ldots, a_k\) and \(b_1,\ldots, b_k\) respectively,
then the covariance \(\sigma_{S_{1}S_{2}}\) is \[
...
\subsection{Directions of extremal variance}\label{sec-extremalvariance}
...
\begin{equation}\phantomsection\label{eq-lagrange}{
S(u, \lambda) = u^{\intercal}D_{0}u - \lambda(u\cdot u -1)
}\end{equation}

Then we compute the gradient

\begin{equation}\phantomsection\label{eq-lagrangegradient}{
...
\caption{Properties of Eigenvalues of Real Symmetric
Matrices}\label{tbl-symmmat}\tabularnewline
\toprule\noalign{}
...
\caption{Simulated Data for PCA
Analysis}\label{tbl-rawdata}\tabularnewline
\toprule\noalign{}
...
}\end{equation} where \(U\) and \(P\) are orthogonal matrices of size
\(N\times N\) and \(k\times k\) respectively, and \(\tilde{\Lambda}\) is
an \(N\times k\) diagonal matrix. This is called the ``singular value
decomposition'' of \(X_{0}\), and the entries of \(\tilde{\Lambda}\) are
called the singular values. If we let \(u_1,\ldots, u_k\) be the first
\(k\) rows of \(U\), then the \(k\) column vectors \(u_{i}^{\intercal}\)
are an orthonormal basis for the feature space spanned by the columns of
\(X_{0}\), and they point in the ``principal directions'' for the data
matrix \(X_{0}\).
...
among \(N\) flips is \begin{equation}\phantomsection\label{eq-binomial}{
...
\begin{equation}\phantomsection\label{eq-multivariategaussian}{
...
\includegraphics{chapters/img/stock_returns.png}
...
\includegraphics{chapters/img/constant_risk_and_line.png}
...
\subsection{Bayesian experiments with the normal
distribution}\label{bayesian-experiments-with-the-normal-distribution-1}
...
\subsection{Bayesian coin flipping}\label{bayesian-coin-flipping-1}
...
\subsection{Bayesian experiments with the normal
distribution}\label{bayesian-experiments-with-the-normal-distribution-2}
...
\subsection{Bayesian coin flipping}\label{bayesian-coin-flipping-2}
...
\] or \begin{equation}\phantomsection\label{eq-ridgeformula}{
(X^{\intercal}X+s)M_r=X^{\intercal}Y
}\end{equation} where \(s=\sigma^2/\tau^2\).

Therefore the ridge coefficients are given by the equation
\begin{equation}\phantomsection\label{eq-ridgecoeffs}{
...
\subsection{Bayesian Regression (or Ridge
Regression)}\label{bayesian-regression-or-ridge-regression-1}
...
\subsubsection{Practical aspects of ridge
regression}\label{practical-aspects-of-ridge-regression-1}
...
\subsection{Bayesian Regression (or Ridge
Regression)}\label{bayesian-regression-or-ridge-regression-2}
...
\subsubsection{Practical aspects of ridge
regression}\label{practical-aspects-of-ridge-regression-2}
...
\subsection{Bayesian experiments with the normal
distribution}\label{bayesian-experiments-with-the-normal-distribution-3}
...
\subsection{Bayesian coin flipping}\label{bayesian-coin-flipping-3}
...
\subsection{Bayesian Regression (or Ridge
Regression)}\label{bayesian-regression-or-ridge-regression-3}
...
\subsubsection{Practical aspects of ridge
regression}\label{practical-aspects-of-ridge-regression-3}
...
\caption{Ocurrences of \textbf{great} by type of
review}\label{tbl-great}\tabularnewline
\toprule\noalign{}
& + & - & total \\
\midrule\noalign{}
\endfirsthead
...
\begin{longtable}[]{@{}llll@{}}
\caption{Ocurrences of \textbf{waste} by type of
review}\label{tbl-waste}\tabularnewline
\toprule\noalign{}
& + & - & total \\
\midrule\noalign{}
\endfirsthead
...
\begin{equation}\phantomsection\label{eq-matrixlikelihood}{
...
\phantomsection\label{alg-gradient_descent}
...
\caption{\label{fig-grad\_descent\_local\_minimum}Gradient descent to a
local minimum}
...
See Equation~\ref{eq-Msolution} and \textbf{?@eq-projection}.
...
\phantomsection\label{alg-graddescentLinearRegression}
...
\phantomsection\label{alg-stochastic-sgd}
...
\begin{equation}\phantomsection\label{eq-logistic_1}{
\log\frac{p}{1-p} = ax+b 
}\end{equation}
...
\caption{\label{fig-logistic\_curve}Logistic Curve}
...
\caption{Sample Data}\label{tbl-logistic_data}\tabularnewline
\toprule\noalign{}
\(x\) & -3 & -2 & -1 & 0 & 1 & 2 & 3 \\
\midrule\noalign{}
\endfirsthead
...
that yield \begin{equation}\phantomsection\label{eq-logistic_a_b}{
p(x) = \frac{1}{1+e^{-ax-b}}
}\end{equation}
...
\begin{equation}\phantomsection\label{eq-logisticregressionlikelihood}{
...
\begin{equation}\phantomsection\label{eq-multiclasslikelihood}{
...
\begin{equation}\phantomsection\label{eq-multiclassgradient}{
...
\phantomsection\label{alg-graddescent}
...
\begin{equation}\phantomsection\label{eq-degreeone}{
...
\] where \begin{equation}\phantomsection\label{eq-kernel}{
\begin{aligned}
...
\end{aligned}
}\end{equation} Thus the function we are trying to minimize is
relatively simple.
...
\includegraphics{chapters/img/neuron.png}
...
\includegraphics{chapters/img/layers.png}
...
\includegraphics{chapters/img/feedforward.png}
...
\includegraphics{chapters/img/linear_regression_network.png}
...
\includegraphics{chapters/img/logistic_network.png}
...
\section{Loss functions and training}\label{loss-functions-and-training}
...
Our neural network is made up of \(n\) layers, with the output of the
final \(n\)th layer serving as input to the loss function. The nodes at
the \(j\)th layer have values \(z^{(j)}_{k}\). The idea behind
...
\includegraphics{chapters/img/backprop_1.png}
...
\includegraphics{chapters/img/backprop-2.png}
...
\begin{equation}\phantomsection\label{eq-z-computation}{
...
\bibitem[\citeproctext]{ref-KaggleFoodData}
\CSLLeftMargin{{[}4{]} }%
...
\bibitem[\citeproctext]{ref-bennettDuality}
\CSLLeftMargin{{[}7{]} }%
...
\bibitem[\citeproctext]{ref-penguindata}
\CSLLeftMargin{{[}9{]} }%
\CSLRightInline{\textsc{Horst}, A. Palmer penguins.Available at
\url{https://github.com/allisonhorst/palmerpenguins}.}

\bibitem[\citeproctext]{ref-plattSMO}
\CSLLeftMargin{{[}10{]} }%
...
\bibitem[\citeproctext]{ref-KernelMethodAdvances}
\CSLLeftMargin{{[}11{]} }%

...
If we really have no opinion about the temperature other than it being between,  say, $-20$ and $100$ degrees,
...
$$
P(\mathbf{t}=\mathbf{t}_{0})=\int P(\mathbf{t}=\mathbf{t}_{0}|t_{*},\sigma^2)P(t_{*},\sigma^2)\,dt_*\,d\sigma^2
$$
...
$$
P(t) = \left(\frac{1}{\sqrt{2\pi}}\right)e^{-(t-t_*)^2/2}.
$$
...
The total probability $T=P(\mathbf{t}=\mathbf{t}_0)$ is hard to calculate, so let's table that for a while.
...
$$
e^{-\left(\|\mathbf{t}-t_{*}\mathbf{e}\|^2/2+(t_{*}-30)^2/30\right)}.
$$
...
and posterior mean
$$
\mu' = \frac{\frac{\mu_0}{\tau^2}+\frac{n}{\sigma^2}\overline{y}}{\frac{1}{\tau^2}+\frac{n}{\sigma^2}}
$$
